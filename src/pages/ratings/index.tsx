...
import { getRatings, deleteRatingById } from 'apiSdk/ratings';
...
function RatingListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('rating', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/ratings/view/${id}`);
    }
  };
  ...
  return (
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('rating', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Flex justifyContent="space-between" mb={4}>
            <Text as="h1" fontSize="2xl" fontWeight="bold">
              Rating
            </Text>
            <NextLink href={`/ratings/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('rating', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/ratings/edit/${record.id}`} passHref legacyBehavior>
                          <Button
